...
removeTranslationItems thy =
  Theory {_thyName=(L.get thyName thy)
    ...
    where
      ...
      removeTranslationElement (VersionItem version) = VersionItem version
...
openTranslatedTheory thy =
  Theory {_thyName=(L.get thyName thy)
    ...
    where
      ...
      addTranslationElement (VersionItem version) = Just $ VersionItem version
...
addAutoSourcesLemmaDiff hnd lemmaName crcLeft crcRight items =
  ...
  where
    ...
    toSide _ (VersionItem _) = Nothing
